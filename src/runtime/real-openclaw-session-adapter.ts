...
export class ExperimentalRealOpenClawSessionAdapter implements OpenClawSessionAdapter {
  constructor(
    private readonly runtime: Pick<PluginRuntime, "config">,
    private readonly config: Pick<SwarmPluginConfig, "acp">,
    private readonly sdkLoader: () => Promise<SdkLike> = async () => (await import("openclaw/plugin-sdk")) as SdkLike,
  ) {}
  ...
  private async getManager(): Promise<{ manager: AcpManager; cfg: unknown }> {
    ...
    if (!manager) {
      throw new Error(
        "OpenClaw plugin-sdk does not expose getAcpSessionManager at runtime; real ACP adapter remains blocked on an upstream public control-plane export",
      );
    }
...
export function createSessionAdapter(
  runtime: Pick<PluginRuntime, "config"> | undefined,
